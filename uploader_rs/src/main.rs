...
use std::fs;
use rfd::FileDialog;
use interprocess::local_socket::{prelude::*, ConnectOptions, GenericFilePath, GenericNamespaced, ListenerOptions, Name};
...
fn local_socket_name(name: &str) -> std::io::Result<Name<'static>> {
    ...
    #[cfg(not(windows))]
    {
        name.to_fs_name::<GenericFilePath>().map(|n| n.into_owned())
    }
}
...
fn main() {
    ...
    if let Ok(img) = image::load_from_memory(icon_bytes) {
        ...
        native_options.viewport.icon = Some(std::sync::Arc::new(egui::IconData { rgba: rgba.into_raw(), width: w, height: h }));
    }
    ...
    let _ = eframe::run_native("Uploader (Rust)", native_options, Box::new(move |cc| {
        ...
        // register embedded image assets (generated by scripts/resize_assets.py)
        assets::register(cc);
