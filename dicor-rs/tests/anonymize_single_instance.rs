...
fn minimal_instance_anonymizes_safely() {
    ...
    let res = anonymize_file(&in_path, &out_dir, false, None).expect("anonymize minimal");
    assert!(res.exists());
}
...
fn issuer_of_patient_id_changed_if_not_empty_and_not_added_if_empty() {
    ...
    let res = anonymize_file(&obj_path, &out_dir, false, None).expect("anonymize case1");
    assert!(res.exists());
    ...
    let res2 = anonymize_file(&in_path2, &out_dir2, false, None).expect("anonymize case2");
    assert!(res2.exists());
